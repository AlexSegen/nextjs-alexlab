...
const NavItem = ({ href, children }: { href: string; children: React.ReactNode }) => (
  <Link href={href} className="block px-6 py-6 font-semibold text-gray-400 hover:text-white">
    {children}
  </Link>
)
...
const Header = () => {
  ...
  return (
    <header
      className={` w-full p-4 lg:p-0 z-40  ${scrollPosition > 400 ? 'bg-gray-900 fixed top-0 fade-in-top' : 'bg-black relative'}`}
    >
            ...
            <MenuItems className="absolute z-40 w-full bg-gray-800 rounded md:hidden -bottom-80 focus:outline-none">
              <nav className="justify-end block w-full md:flex">
                ...
                <NavItem href="/portfolio">
                  <i className="hidden w-3 h-3 mr-2 bg-green-400 rounded-full opacity-75 md:inline-flex animate-ping"></i> Portfolio
                </NavItem>
